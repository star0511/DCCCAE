...
def preprocess_signature(img: np.ndarray,
    ...
    centered = normalize_image(img, img_size, canvas_size)

    if input_size is not None and input_size != img_size:
        cropped = crop_center(centered, img_size)
    else:
        cropped = centered
...
def normalize_image(img: np.ndarray,
    ...
    cropped = binarized_image[r.min(): r.max(), c.min(): c.max()]
    # plt.figure()
    # plt.axis('off')
    # plt.imshow(cropped)
    # plt.show()
    ...
    normalized_image = np.ones((max_rows, max_cols), dtype=np.uint8)
    # Add the image to the blank canvas
    normalized_image[r_start:r_start + img_rows, c_start:c_start + img_cols] = cropped

    normalized_image = transform.resize(normalized_image, imgsize)
    threshold = filters.threshold_otsu(normalized_image)

    # Find the center of mass
    normalized_image = normalized_image > threshold
