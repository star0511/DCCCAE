...
def exthogfeeature():
    filelist = ["../data/SigComp2011.npz", "../data/CEDAR.npz", "../data/UTSig.npz", "../data/BHSigH.npz",
                "../data/BHSigB.npz"]
    # filelist = ["../data/BHSigB.npz","../data/SigComp2011.npz"]
    ...
    for file in filelist:
        ...
        # print(pca_feature_lbp)
        print("特征提取完毕")

        pca = PCA(n_components=256)  # 实例化
        pca = pca.fit(pca_feature_lbp[:800])  # 拟合模型
        print("模型拟合完毕")
        pca_feature_lbp = pca.transform(pca_feature_lbp)
        # for i in tqdm(range(len(pca_feature_lbp))):
        #     pca_feature_lbp[i] = pca.transform([pca_feature_lbp[i]])[0]
        ...
        np.savez(file.replace(".npz","_hand_norm").replace("data","data/glcm").replace("maxsize",""),
                 x=x,
                 y=y,
                 yforg=yforg)
def plbp(img,i,l):
    ...
    glcm = np.mean(graycomatrix(img, [1], [0, np.pi/2,np.pi/4,np.pi*3/4]),axis=3).flatten()
    return glcm
# 填充缺失值
