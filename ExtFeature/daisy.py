...
# 提取图像的daisy特征
def daisy_feature(image_list):
    feature_daisy_list = []  # daisy特征向量列表
    p = Pool(100)
    for i in range(len(image_list)):
        # 获取sift特征，kp为关键点信息，des为关键点特征矩阵形式
        feature_daisy_list.append(p.apply_async(pdaisy, args=(image_list[i][0], i, len(image_list))))
    p.close()
    p.join()
    for i in tqdm(range(len(feature_daisy_list))):
        feature_daisy_list[i]=feature_daisy_list[i]._value
    return np.array(feature_daisy_list)
def pdaisy(img,i,lenimg):
    print("\rdaisy(img)[{}/{}]".format(i,lenimg),end=" ")
    # sift = cv.SIFT_create()
    # kp, des = sift.detectAndCompute(img, None)
    return daisy(img)
# 初始化BOW训练器
def bow_init(feature_daisy_list):
    # 创建BOW训练器，指定k-means参数k   把处理好的特征数据全部合并，利用聚类把特征词分为若干类，此若干类的数目由自己设定，每一类相当于一个视觉词汇
    bow_kmeans_trainer = cv.BOWKMeansTrainer(128)

    for feature_daisy in feature_daisy_list:
        bow_kmeans_trainer.add(feature_daisy)
    ...
    daisy = cv.SIFT_create()
    bow_img_descriptor_extractor = cv.BOWImgDescriptorExtractor(daisy, flann)
