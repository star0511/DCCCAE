...
def autoencoder(inputs, D_in, layer_widths, layer_activations, name, variable_reuse, initializer):
...
class DCCCAE(object):
    ...
    def __init__(self, filename, architecture,rcov1=0, rcov2=0, learning_rate=0.0001, l2_penalty=0.0, dccaelamda=0.0001,dcccaelamda=100):
        # Save the architecture and parameters.
        self.network_architecture=architecture
        self.l2_penalty=l2_penalty
        self.dcccaelamda = dcccaelamda
        ...
        self.FX1 = autoencoder(self.x1, self.n_input1, architecture["F_hidden_widths"],
                               architecture["F_hidden_activations"], "F", None, initializer)
        self.FX2 = autoencoder(self.x2, self.n_input2, architecture["G_hidden_widths"],
                               architecture["G_hidden_activations"], "G", None, initializer)
        ...
        self.Jacobian = self.dcccaelamda * (tf.reduce_sum(tf.square(tf.gradients(self.FX1[0], self.x1, stop_gradients = [self.x1]))) + tf.reduce_sum(tf.square(tf.gradients(self.FX2[0], self.x2, stop_gradients = [self.x2]))))
        # Weight decay.
        # self.weightdecay=tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables()])
        # Define cost and use the ADAM optimizer.
        self.cost = self.ccaloss + self.aeloss + self.Jacobian# + l2_penalty * self.weightdecay
        LEARNING_RATE_DECAY = 0.5  # 学习率衰减率
        LEARNING_RATE_STEP = 20  # 喂入多少轮BATCH_SIZE后，更新一次学习率，一般设为：总样本数/BATCH_SIZE
    ...
    def partial_fit(self, X1, X2):
        opt, FX1, FX2,aeloss, ccaloss, cost = self.sess.run([self.optimizer,self.FX1,self.FX2,self.aeloss, self.ccaloss, self.cost], feed_dict={self.x1:X1, self.x2:X2, self.batchsize: X1.shape[0]})
...
def train(model, trainData, tuneData, batch_size=100, max_epochs=10):
    ...
    while epoch < max_epochs:
        ...
        if batch_size == 0:
            ...
            ae_avg_cost,cca_avg_cost,avg_cost = model.partial_fit(batch_x1, batch_x2)
            ...
            for i in range(total_batch):
                ...
                # Fit training using batch data.
                aecost,ccacost,cost=model.partial_fit(batch_x1, batch_x2)
