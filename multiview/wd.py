...
def main(args,filename):
    ...
    svmusernumtlist = {"cedar": [12, 10, 12], "utsig": [10, 10, 12], "bhsigb": [10, 10, 8], "bhsigh": [10, 10, 8],
                       "sigcomp2011": [10, 10, 8]}
    forg_from_dev = svmusernumtlist[args.datasetname][0]
    gen_for_test = svmusernumtlist[args.datasetname][1]
    if args.gen_for_train==0:
        gen_for_train = svmusernumtlist[args.datasetname][2]
        wdfunc(args,filename,forg_from_dev,gen_for_test,gen_for_train)
    else:
        for i in range(1,svmusernumtlist[args.datasetname][2]+1):
            gen_for_train = i
            wdfunc(args,filename,forg_from_dev,gen_for_test,gen_for_train)
def wdfunc(args,filename,forg_from_dev,gen_for_test,gen_for_train):
    datasetlist = {"cedar": 0, "utsig": 1, "bhsigb": 2, "bhsigh": 3, "sigcomp2011": 4}
    data = np.load(filename, allow_pickle=True)[datasetlist[args.datasetname]]
    usernumtlist = {"cedar": 55, "utsig": 115, "bhsigb": 100, "bhsigh": 160,"sigcomp2011":20}
    exp_users = range(1, usernumtlist[args.datasetname]+1)
    dev_users = range(1, usernumtlist[args.datasetname]+1)

    features = data['development'].real
    y = data['devy'].ravel()
    yfrog = data['devlabel'].ravel()
    ...
    viz.text(str(args)+'<br>FRR: {:.2f} (+- {:.2f})<br>'.format(np.mean(frr_list) * 100, np.std(
        ...
        eer_list) * 100) + 'EER (user thresholds): {:.2f} (+- {:.2f})<br>'.format(np.mean(eer_u_list) * 100,
             ...
             win=str(args))
    fnew = open(str(args.eername)+".txt", "a", encoding="utf-8")
    fnew.write('\n\n\n' + str(args) + '\n')
